...
import { APPLE_STORE_APPID } from "../services/constants";
...
export class VersionProvider {
  ...
  init() {
    ...
    if (this.isMobile) {
      this.nativeService.getVersionNumber().subscribe(currentVersionNo => {
        ...
        this.storageService.setItem("version", this.currentVersionNo);
      });
      this.getLastestVersion().subscribe((data) => {
        //this.nativeService.alert("version data is",JSON.stringify(data));
      },err => {
  ...
  //       //从后台查询app最新版本信息
  //       let url = Utility.formatUrl(`${APP_VERSION_SERVE_URL}/v1/apply/getDownloadPageByEName/${this.appName}/${this.appType}`);
  ...
  //       const apk = this.file.externalRootDirectory + 'download/' + `android_${Utility.getSequence()}.apk`; //apk保存的目录
  //       //下载并安装apk
  ...
  // getVersionList() {
  //   if (this.isMobile) {
  //     let url = Utility.formatUrl(`${APP_VERSION_SERVE_URL}/v1/apply/findVersionList/${this.appName}/${this.appType}`);
  ...
  /* getLastestVersion() {
    let versionType = "";
    if(this.nativeService.isMobile() && this.nativeService.isAndroid()){
      versionType = "Android";
    }else if(this.nativeService.isMobile() && this.nativeService.isIos()){
      versionType = "iOS";
    }
  
    let seq = this.httpService.post('Version/LatestVersion', true, {"versionType": versionType}).share();
    seq.subscribe((data: any) => {
      if (data.errcode == 0) {
        this.latestVersionNo = data.result.versionNumber;
        this.lastVersionInfo = data.result;
      }
    }, err => {
      console.error('ERROR', err);
    });
  ...
  getLastestVersion() {
    ...
    if(this.nativeService.isAndroid()){
      let seq = this.httpService.post('Version/LatestVersion', true, {"versionType": 'Android'}).share();
      seq.subscribe((data: any) => {
        if (data.errcode == 0) {
          this.latestVersionNo = data.result.versionNumber;
          this.lastVersionInfo = data.result;
          this.appDownloadPageUrl = "http://ifih-t.fih-foxconn.com/";
        }
      }, err => {
        console.error('ERROR', err);
      });
      ...
      let url =  'https://itunes.apple.com/lookup?id=' + APPLE_STORE_APPID;
      let seq = this.httpService.post(url, true, {"id": '284910350'}).share();
      seq.subscribe((data: any) => {
      ...
      return seq;
        ...
        //调用苹果提供的API获取版本号
        //测试
       let url =  'https://itunes.apple.com/lookup?id=284910350'; 
       let seq = this.httpService.post(url, true, {"id": '284910350'}).share();
       seq.subscribe((data: any) => {
         if (data.errcode == 0) {
